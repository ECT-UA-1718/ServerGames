 ...
 * O seu objetivo é ter funções que pedem ao user dados de um certo tipo e só devolvem o valor quando o input do cliente for válido
...
public class Get {
    ...
    public static int getIntInRangeFromClient(User user, int min, int max) throws IOException{
        boolean cond = false;
        String response = "";
        do {
            ...
            if(!cond){
                user.send("Error! Enter a valid number: ");
            }
        } while(!cond);
